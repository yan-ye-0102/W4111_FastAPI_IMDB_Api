...
class MySQLDataService(BaseDataService):
    ...
    def build_select(database, collection, predicate, project):
        ...
        sql = "select " + select_clause + " from " + database + "." + collection + wc
    ...
    @staticmethod
    def build_create(database, collection, new_data):
        """
    ...
    def update(self, database, collection, predicate, new_data):
        ...
        """
        conn = self.get_connection()
        sql, args = self.build_update(database, collection, predicate, new_data)
        result = self.run_q(sql, args, conn, False)
        return result
